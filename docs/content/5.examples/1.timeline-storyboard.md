---
title: Timeline storyboard
description: Independent timelines synced into one composition.
navigation:
  icon: i-ph-film-strip
---

Four cards each perform their own signature move. Together they join into
one composition that can be played, paused, restarted, or scrubbed from a
single control, the way an editor cuts four separately-shot clips onto one
timeline.

::render-code-block-preview
---
code: false
src: examples/showcase/timeline-storyboard/TimelineStoryboardDemo.vue
---
::

## What's happening

Each card plays its own short move, like four dancers who each learned a
separate routine. Those routines are lined up on one shared timeline, the
way a video editor lines up four clips on one track. Drag the scrub bar and
you move through all four at once, so halfway through you might catch Card
02 mid-flip while Card 03 hasn't started yet.

## How it's built

The goal was to write and tune each card's routine without worrying about
the other three, so each one gets its own
[`useAnimeTimeline`](/composables/use-anime-timeline), built with
`autoplay: false` so it sits ready but doesn't play on its own. Once a
routine looked right in isolation, it got dropped onto a fifth, master
timeline using `.sync(childTimeline, offsetMs)`, where `offsetMs` is just
"how far into the master should this card's routine start." The offsets are
staggered (0ms, 350ms, 850ms...), so each card starts a beat after the last
one instead of all four starting together.

From there, the play, pause, and scrub controls only ever talk to that one
master timeline. Calling `.seek(progress * duration)` on it is enough to
scrub every card at once, because they're all riding the same clock.

## Building it yourself

1. Give each card its own [`useAnimeTimeline`](/composables/use-anime-timeline) instance, created with `autoplay: false`, and author its keyframes independently of the others.
2. Optionally split each card into two nested elements: an outer "mover" `div` for `y` (position), and an inner "rotator" `div` for `scale` and `rotate`. This isn't required. Anime.js keeps one shared transform cache per element, so two separate `.add()` calls for position and rotation would still merge cleanly into one `transform` even on a single `div`. The split is there for organization: position steps and rotation steps rarely share the same step count or duration, so giving each its own element keeps the two `.add()` calls independent and easy to read, instead of forcing one keyframe schedule to fit both.
3. Create one more `useAnimeTimeline`, also `autoplay: false`, to act as the master. Give it `onUpdate` for progress reporting and `onBegin` / `onPause` / `onComplete` for play state.
4. Call `.sync(childTimeline, offsetMs)` on the master for each card, staggering the offsets so the routines play in sequence rather than together.
5. Wire Play, Pause, and Restart buttons to the master's `.play()`, `.pause()`, and `.restart()`, and a range slider to `.seek(value * master.duration)`.

::collapsible{name="AI build prompt"}
```text
Using the nanime Nuxt module, build a Vue 3 component with 4 card elements.
Give each card its own useAnimeTimeline instance (autoplay: false) with a
distinct multi-step animation (position, rotation, scale) using arrays of
keyframes with an individual duration and ease per step. Use Anime.js's own
easing names, like 'outQuad', 'outBounce', 'outElastic', 'inOutSine', and
'linear'. Skip the 'easeOutQuad' or 'easeInOutSine' naming convention some
other animation libraries use. An unrecognized ease string fails silently
and falls back to linear, so get this from the module's actual easing list
rather than guessing. Optionally split each card into a "mover" element for
position and a "rotator" element for scale and rotation, purely to keep
each `.add()` call's keyframe schedule independent since they rarely share
the same step count or duration.

Create a master useAnimeTimeline (autoplay: false) with onUpdate reporting
progress from 0 to 1, and onBegin / onPause / onComplete toggling an
isPlaying flag. Sync each card's timeline into the master with
.sync(childTimeline, offsetMs), staggering the offsets so the cards animate
one after another instead of all at once.

Add Play, Pause, and Restart buttons calling the master's .play(),
.pause(), and .restart(), and a range slider that calls
.seek(value * master.duration) to scrub through the whole composition.
```
::
